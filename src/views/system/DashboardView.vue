...
const stats = computed(() => [
  { title: 'Total Books', value: bookStore.totalBooks, icon: 'mdi-book-multiple', color: 'primary' },
  { title: 'Available Books', value: bookStore.availableBooks, icon: 'mdi-book-open-variant', color: 'success' },
  { title: 'Borrowed Books', value: bookStore.borrowedBooks, icon: 'mdi-book-lock', color: 'warning' },
  { title: 'My Borrowings', value: userBorrowings.value.length, icon: 'mdi-account-box', color: 'info' }
])
  ...
  <AppLayout>
    <template #content>
      <v-container fluid>
        ...
        <!-- Welcome Banner -->
        <v-sheet color="primary" class="welcome-banner pa-6 mb-6 rounded text-white">
          <h1 class="text-h4 mb-2">Welcome to Book Borrow System</h1>
          <p class="text-subtitle-1">Hello, {{ authStore.userData?.firstname }}! Manage your book borrowings efficiently.</p>
        </v-sheet>
            ...
            <v-table fixed-header height="400px">
              ...
              <tbody>
                <tr v-for="book in filteredBooks" :key="book.id">
                  ...
                  <td>{{ book.author }}</td>
                  <td>
                    <v-chip size="small" color="primary" variant="outlined">{{ book.category }}</v-chip>
                  </td>
                  <td>
                    <v-chip
                      size="small"
            ...
            <v-table fixed-header height="400px">
              ...
              <tbody>
                <tr v-for="borrowing in userBorrowings" :key="borrowing.id">
                  ...
                  <td>
                    <v-chip size="small" color="primary" variant="outlined">{{ borrowing.books.category }}</v-chip>
                  </td>
                  ...
                  <td>
                    ...
                    <span v-else>Returned on {{ new Date(borrowing.return_date).toLocaleDateString() }}</span>
                  </td>
                </tr>
              ...
              <v-container v-if="selectedBook">
                ...
                <v-row>
                  <v-col cols="12">
                    ...
                    <v-chip size="small" color="primary" class="mb-4">{{ selectedBook.category }}</v-chip>
                  </v-col>
                </v-row>
